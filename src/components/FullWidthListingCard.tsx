...
export default function FullWidthListingCard({
  ...
  return (
    <Card className={cn("flex w-full max-w-4xl overflow-hidden rounded-3xl", className)}>
      <div className="relative w-2/5 min-h-[300px] group z-10">
        <Image
      ...
      <CardContent className="flex flex-col justify-between w-3/5 p-6">
        <div className="space-y-4">
          ...
          <Link href={`/listings/${slug}`} className="block">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>{location}</span>
              <span>{formatDistance(new Date(timestamp), new Date(), { addSuffix: true })}</span>
            </div>
          </Link>
        </div>
