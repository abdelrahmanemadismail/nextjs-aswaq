...
export default function FullWidthListingCard({
  ...
  const mileage = listing.vehicle_details?.mileage 
    ? `${listing.vehicle_details.mileage.toLocaleString()} km`
    : undefined
      ...
      <CardContent className="flex flex-col justify-between w-full md:w-3/5 p-4 md:p-6">
        <div className="space-y-3 md:space-y-4">
          ...
          <Link href={`/listings/${listing.slug}`} className="block">
            <p className="text-lg md:text-xl text-muted-foreground line-clamp-2">{listing.title}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-4 md:mt-6">
          ...
          {(!listing.contact_methods || listing.contact_methods.includes('phone')) && (
            <Button 
              ...
              title={!listing.phone_number ? "Phone number not available" : "Call seller"}
            >
            ...
            <Button 
              ...
              onClick={() => {
                if (listing.phone_number) {
                  ...
                  const message = `Hi, I'm interested in your listing: "${listing.title}" on Aswaq`;
                  const whatsappUrl = `https://wa.me/${formattedNumber}?text=${encodeURIComponent(message)}`;
                  window.open(whatsappUrl, '_blank');
                }
              }}
              disabled={!listing.phone_number}
              title={!listing.phone_number ? "WhatsApp number not available" : "Message on WhatsApp"}
            >
